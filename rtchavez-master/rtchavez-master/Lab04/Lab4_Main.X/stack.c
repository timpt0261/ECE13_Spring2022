...
int StackPush(struct Stack *stack, double value) {
    ...
    //check that the stack is Full or uninitialized
    if (StackIsFull(stack) || !stack->initialized) {
 ...
 *   *value = 7.0;
 * This is because "value" is a pointer. K&R 5.1-5.2 should explain this practice of 
 * "passing by reference". 
 */
int StackPop(struct Stack *stack, double *value) {

    // Check stack is not empty or uninitialized
    if (StackIsEmpty(stack) || !stack->initialized) {
        return STANDARD_ERROR;
    }
    // store popped value
    *value = stack->stackItems[stack->currentItemIndex];
    // remove value at end of stack
...
int StackGetSize(const struct Stack *stack) {
    return stack->initialized ? stack->currentItemIndex + 1 : SIZE_ERROR;
}